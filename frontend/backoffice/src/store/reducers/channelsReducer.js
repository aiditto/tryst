...
const patchChannel = state => {
  return updateObject(state, { loading: true, error: null });
};

const patchChannelSuccess = state => {
  return updateObject(state, { loading: false, error: null });
};

const patchChannelFailure = (state, action) => {
  return updateObject(state, { loading: false, error: action.error });
};
...
const channelsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.PATCH_CHANNEL:
      return patchChannel(state);

    case actionTypes.PATCH_CHANNEL_SUCCESS:
      return patchChannelSuccess(state);

    case actionTypes.PATCH_CHANNEL_FAILURE:
      return patchChannelFailure(state, action);
